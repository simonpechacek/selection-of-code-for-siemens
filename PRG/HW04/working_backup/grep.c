...
int main(int argc, char *argv[])
{
  ...
  if (!err)
  {
    ...
    if (input_file != NULL)
      fclose(input_file);
    if (!regular_expresion && !color_match)
    {
      for (int i = 0; i < num_lines; ++i)
      {
        bool matched_line = match(input[i], pattern);
        if (matched_line)
          printf("%s", input[i]);
...
bool match(const char *str, const char *pattern)
{
  ...
  for (int i = 0; i <= s_len - pattern_len; ++i)
  {
    int counter = 0;
    bool match = false;
    while (str[i + counter] == pattern[counter])
    {
  ...
  for (int i = string_len - 1; i >= 0; --i)
  {
    ...
    while (true)
    {
      ...
      else
      {
        ...
        if (!reg_type)
          break;
        int index_offset = (*regular_expressions[reg_type - 1])(str, pattern[pattern_len - 2 - counter_pattern], i - counter_string);
        if (index_offset < 0)
          break;
        counter_pattern += 2;
        counter_string += index_offset;
        index_len += index_offset;
        // printf("index_offeset: %d || %c || %c\n",index_offset,  str[i - counter_string], pattern[pattern_len - 1 - counter_pattern]);
      }
